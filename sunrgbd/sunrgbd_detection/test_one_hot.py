...
NUM_CHANNEL = 6
TEST_DATASET = roi_seg_box3d_dataset.ROISegBoxDataset(npoints=NUM_POINT, split='val', rotate_to_center=True,
...
def write_detection_results(result_dir, id_list, type_list, box2d_list, center_list, heading_cls_list, heading_res_list,
...
def main_batch_from_rgb_detection(output_filename, result_dir=None):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_rot_angle, batch_rgb_prob, batch_one_hot_vec = get_batch(TEST_DATASET, test_idxs, start_idx,
                                                                                   end_idx, NUM_POINT, NUM_CHANNEL,
    ...
    if FLAGS.dump_result:
        save_zipped_pickle(
            [ps_list, segp_list, center_list, heading_cls_list, heading_res_list, size_cls_list, size_res_list,
             rot_angle_list, score_list], output_filename)

        # Write detection results for KITTI evaluation
    print(len(ps_list))
    raw_input()
    write_detection_results(result_dir, TEST_DATASET.id_list, TEST_DATASET.type_list, TEST_DATASET.box2d_list,
                            center_list, heading_cls_list, heading_res_list, size_cls_list, size_res_list,
